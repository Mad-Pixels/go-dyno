...
func ValidateOperator(dynamoType string, op OperatorType) bool {
    ...
    case "S": // String
        return op == EQ || op == GT || op == LT || op == GTE || op == LTE || 
               op == BETWEEN || op == CONTAINS || op == BEGINS_WITH || op == IN || op == NOT_IN ||
    ...
    default:
        // Для неизвестных типов разрешаем только базовые операции
        return op == EQ || op == EXISTS || op == NOT_EXISTS
    }
}
...
func BuildConditionExpression(field string, op OperatorType, values []interface{}) (expression.ConditionBuilder, error) {
    ...
    case IN:
        valueList := make([]expression.OperandBuilder, len(values))
        for i, v := range values {
            valueList[i] = expression.Value(v)
        }
        return fieldExpr.In(valueList[0], valueList[1:]...), nil
    case NOT_IN:
        // NOT IN не поддерживается напрямую в DynamoDB, используем NOT(IN(...))
        valueList := make([]expression.OperandBuilder, len(values))
        for i, v := range values {
            valueList[i] = expression.Value(v)
        }
        inCondition := fieldExpr.In(valueList[0], valueList[1:]...)
        return expression.Not(inCondition), nil
    case EXISTS:
